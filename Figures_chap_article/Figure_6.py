...
fig.subplots_adjust(left=0.07, bottom=0.2, right=0.99, top=0.98, wspace=0.3)
fig.set_size_inches(size_fig_article_6)
...
ax_0.errorbar(delta_sig_binned,1/np.sqrt(ell_binned),
                xerr = delta_sig_binned_stds,
                yerr = ell_binned_stds/(2 * (ell_binned)**(3/2)),
                fmt=" ",capsize=error_bar_width,color="k",
                ecolor=error_bar_color,
                elinewidth=.5,alpha=error_bar_alpha,markeredgewidth=markeredgewidth)

ax_0.scatter(delta_sig_binned,1/np.sqrt(ell_binned),
        c=main_plot_color,s=scatter_size*2,
        marker="d",zorder=2,
        edgecolors="k",linewidth=0.01)


ax_0.errorbar(delta_sig_solid,1/np.sqrt(ell_solid),
                xerr = delta_sig_solid_stds,
                yerr = ell_solid_stds/(2 * (ell_solid)**(3/2)),
                fmt=" ",capsize=error_bar_width,color="k",
                ecolor=error_bar_color,
                elinewidth=.5,alpha=error_bar_alpha,markeredgewidth=markeredgewidth)


ax_0.scatter(delta_sig_solid,1/np.sqrt(ell_solid),
        c=solid_solid_color,s=scatter_size*4/3,
        marker="o",zorder=2,
        edgecolors="k",linewidth=0.01)
...
ax_0.set_xlim([0,0.55])
ax_0.set_xticks([0,0.2,0.4])
ax_0.set_xticklabels([0,0.2,0.4])
ax_0.set_ylim([0,5.8])

ax_0.set_xlabel(r"$\left\langle\sigma_{xy}^0-\sigma_{xy}^\infty\right\rangle$ (MPa)")
ax_0.set_ylabel(r"$1/\sqrt{\ell_{slip}}$")
